...
impl Game {
    ...
    fn final_round(&mut self, player: &mut Player) -> Result<(), GameError> {
        let player_move = self.player_move()?;
        match player_move {
            PlayerMove::Hit => {
                self.deal_card(&player.to_string());
                player.hand.add_card(self.deck.remove_card()?);
                self.process_blackjack(player); // refactor process_blackjack and pattern matching
                Ok(())
            }
            PlayerMove::Stand => todo!(),
        }
    }
}
